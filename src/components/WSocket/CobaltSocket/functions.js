...
import { watchstoneApi } from "../../Watchstones/WCobalt/api";

export const watchstones = (id, hh, te, lp, le, nv, gc, vr, la) => {
	if (id === "HH_cobalt_watchstone_socket") {
		return watchstoneApi()
			...
			.map(watchstone => (
				<img
					...
					style={{ width: "11px", height: "11px", top: "18.5px", left: "10.5px" }}
				></img>
			));
	}

	if (id === "TE_cobalt_watchstone_socket") {
		return watchstoneApi()
			.filter(() => te === true)
			.map(watchstone => (
				<img
	...
	if (id === "LP_cobalt_watchstone_socket") {
		return watchstoneApi()
			.filter(() => lp === true)
			.map(watchstone => (
				<img
	...
	if (id === "LE_cobalt_watchstone_socket") {
		return watchstoneApi()
			.filter(() => le === true)
			.map(watchstone => (
				<img
	...
	if (id === "NV_cobalt_watchstone_socket") {
		return watchstoneApi()
			.filter(() => nv === true)
			.map(watchstone => (
				<img
	...
	if (id === "GC_cobalt_watchstone_socket") {
		return watchstoneApi()
			.filter(() => gc === true)
			.map(watchstone => (
				<img
	...
	if (id === "VR_cobalt_watchstone_socket") {
		return watchstoneApi()
			.filter(() => vr === true)
			.map(watchstone => (
				<img
	...
	if (id === "LA_cobalt_watchstone_socket") {
		return watchstoneApi()
			.filter(() => la === true)
			.map(watchstone => (
				<img
