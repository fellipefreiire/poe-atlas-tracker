...
import { watchstoneApi } from "../../Watchstones/WViridian/api";

export const watchstones = (id, hh, te, lp, le, nv, gc, vr, la) => {
	if (id === "HH_viridian_watchstone_socket") {
		return watchstoneApi()
			...
			.map(watchstone => (
				<img
					...
					style={{ width: "11px", height: "11px", top: "10.5px", left: "19.5px" }}
				></img>
			));
	}

	if (id === "TE_viridian_watchstone_socket") {
		return watchstoneApi()
			.filter(() => te === true)
			.map(watchstone => (
				<img
	...
	if (id === "LP_viridian_watchstone_socket") {
		return watchstoneApi()
			.filter(() => lp === true)
			.map(watchstone => (
				<img
	...
	if (id === "LE_viridian_watchstone_socket") {
		return watchstoneApi()
			.filter(() => le === true)
			.map(watchstone => (
				<img
	...
	if (id === "NV_viridian_watchstone_socket") {
		return watchstoneApi()
			.filter(() => nv === true)
			.map(watchstone => (
				<img
	...
	if (id === "GC_viridian_watchstone_socket") {
		return watchstoneApi()
			.filter(() => gc === true)
			.map(watchstone => (
				<img
	...
	if (id === "VR_viridian_watchstone_socket") {
		return watchstoneApi()
			.filter(() => vr === true)
			.map(watchstone => (
				<img
	...
	if (id === "LA_viridian_watchstone_socket") {
		return watchstoneApi()
			.filter(() => la === true)
			.map(watchstone => (
				<img
