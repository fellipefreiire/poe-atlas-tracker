...
import { watchstoneApi } from "../../Watchstones/WCrimson/api";

export const watchstones = (id, hh, te, lp, le, nv, gc, vr, la) => {
	if (id === "HH_crimson_watchstone_socket") {
		return watchstoneApi()
			...
			.map(watchstone => (
				<img
					...
					style={{ width: "11px", height: "11px", top: "3px", left: "10.5px" }}
				></img>
			));
	}

	if (id === "TE_crimson_watchstone_socket") {
		return watchstoneApi()
			.filter(() => te === true)
			.map(watchstone => (
				<img
	...
	if (id === "LP_crimson_watchstone_socket") {
		return watchstoneApi()
			.filter(() => lp === true)
			.map(watchstone => (
				<img
	...
	if (id === "LE_crimson_watchstone_socket") {
		return watchstoneApi()
			.filter(() => le === true)
			.map(watchstone => (
				<img
	...
	if (id === "NV_crimson_watchstone_socket") {
		return watchstoneApi()
			.filter(() => nv === true)
			.map(watchstone => (
				<img
	...
	if (id === "GC_crimson_watchstone_socket") {
		return watchstoneApi()
			.filter(() => gc === true)
			.map(watchstone => (
				<img
	...
	if (id === "VR_crimson_watchstone_socket") {
		return watchstoneApi()
			.filter(() => vr === true)
			.map(watchstone => (
				<img
	...
	if (id === "LA_crimson_watchstone_socket") {
		return watchstoneApi()
			.filter(() => la === true)
			.map(watchstone => (
				<img
